...
const Segmented = ({ options, current, setCurrent }) => {
  return (
    <div>
      <div className="inline-flex h-9 w-full items-baseline justify-start rounded-lg bg-gray-100 text-black p-1 sm:w-auto shadow-gray">
        {options &&
          options.length > 0 &&
          options.map((option, index) => (
            <button
              ...
              className={cn({
                ...
                "rounded-lg  bg-transparent text-slate-500": current !== option.value,
              })}
            >
              {option.name}
            </button>
