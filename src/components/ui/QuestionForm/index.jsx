    ...
    return (
        <>
            <div className="mt-2 flex flex-wrap justify-between items-center gap-2">
                {/* question type */}
                <div className="flex rounded-lg border border-gray-300 shadow-sm text-sm font-bold divide-x divide-gray-300 text-black">
                    {questionTypes.map(({ type }, index) => (
                        <div
                            key={index}
                            onClick={() => changeType(type)}
                            className={`flex gap-2 items-center min-w-fit px-4 py-2  hover:cursor-pointer hover:bg-violet-600 ${selectedQuestion.type === type && "bg-violet-600 text-white"}`}
                        >
                            <div
                                className={classNames(
                                    `w-4 h-4 rounded-full border-2 border-gray-400 hover:cursor-pointer`,
                                    selectedQuestion.type === type && "bg-white"
                                )}
                            />
                            {type}
                        </div>
        ...
        <div className="mt-2 w-full">
            ...
            <div className="flex gap-2">
                ...
                <div className="flex-1 flex flex-col gap-1 my-1">
                    ...
                    <div>
                        <textarea
                            className="block shadow-sm border-[1px] border-gray-300 text-gray-900 sm:text-sm rounded-md input-transition focus:outline-none focus:ring-0 focus:border-violet-600 block w-full p-3 my-2 bg-white capitalize"
                            type="text"
                            value={desc}
                            onChange={(e) => {
                                const tempContent = { ...content };
